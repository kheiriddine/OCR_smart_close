...
def run_llama_ocr(pdf_dir, output_file, temp_images_dir, ocr_api_key):
    try:
        subprocess.run(["node", "llama_ocr.js", pdf_dir, output_file, temp_images_dir, ocr_api_key], check=True)
        return output_file  # Return the path to the OCR output file
...
def extract_info_from_ocr(ocr_output_file, together_api_key):
    ...
    output_file_path = "extracted_output_VF.json"
